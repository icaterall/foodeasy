...
class UserAPIController extends Controller
{
 ...
 /**
    ...
    function verifyCode(Request $request)
    {
        ...
        return $this->sendResponse($user, 'Activation code sent successfully');
    }
 ...
 /**
     ...
     * @return
     */
    function getSmsCode(Request $request)
    {
        try {
            $this->validate($request, [
                'mobile' => 'required',
            ...
            $code = rand(1000, 9999);
            $find_user = User::where('mobile',$request->input('mobile'))->first();
          ...
          if($user != null)
            ...
            SendSMS::sendUserSms($user->mobile,$user->activation_code); // send and return its response
 ...
 /**
     ...
     * @return
     */
    function getUserPayment(Request $request)
    {
        try {
            $this->validate($request, [
                 'api_token' => 'required'
           ...
           { 
            ...
            if(count($cart)>0)
            return $this->sendResponse('https://spoongate.com/checkout/app_online_banking/'.$user['id'], 'User found successfully');
         else  return $this->sendError('There are no items in the cart for this user', 401);
           } 


           else  return $this->sendError('something went error', 401);
